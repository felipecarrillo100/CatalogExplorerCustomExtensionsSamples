...
const CreateCircleWidgetForm : React.FC<Props> =(props) => {
    ...
    const onSubmit = ()=>{
        ...
        const jsonFeatureCollection = generateNewCircle(inputs.name, widget.current.formData.point.coordinates[0], widget.current.formData.point.coordinates[1], inputs.radius);
...
function generateNewCircle(name: string, lon: number, lat: number, radius: number) {
    const radiusKm = radius;
    const numberOfPoints = 50;
    const res = polygonGenerator.generate(lat, lon, radiusKm, numberOfPoints) as any;
    res.features[0].id = 1;
    res.features[0].properties["name"] = name;
    return res;
}
